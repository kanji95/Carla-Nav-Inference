...
def log_video_predicitons(front_cam_video, lang_command, pred_mask, traj_mask, gt_mask, gt_traj_mask, episode_num, sample_idx, title="train", k=4
    ...
    mask_pred = np.uint8(pred_mask[0]*255.)    
    mask_gt = np.uint8(gt_mask[0]*255.)
    ...
    # indices = np.random.choice(range(pred_mask.shape[1]), size=k, replace=True)
    ...
    #     orig_video = front_cam_video[0, index]
    #     orig_video = rearrange(orig_video, "t h w c -> t c h w")

    #     mask_pred = rearrange(pred_mask[0, index], "c t h w -> t c h w")
