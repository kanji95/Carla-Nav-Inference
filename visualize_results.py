...
network = IROSBaseline(visual_encoder, hidden_dim=256, mask_dim=112)
...
for episode in episodes:
    ...
    for image_file, mask_file in zip(image_files, mask_files):
        ...
        frame = img_transform(image).cuda(non_blocking=True)
        gt_mask = mask_transform(gt_mask).cuda(non_blocking=True)
    ...
    gt_mask_video = np.stack(gt_mask_video, axis=0)

    wandb.log(
        {"video": wandb.Video(frame_video, fps=4, format="gif")},
        {"pred_mask": wandb.Video(mask_video, fps=4, format="gif")},
        {"gt_mask": wandb.Video(gt_mask_video, fps=4, format="gif")},
        {"command": command},
    )
