...
for i in range(len(episodes)):
    os.system(f"python inference_model.py --img_backbone vit_small_patch16_224 --hidden_dim 384 --image_dim 224 --mask_dim 224 --traj_dim 224 --sync --threshold 0.00005 \
        --checkpoint {checkpoint} --glove_path E:/carla/carla/CARLA_0.9.12/glove/glove/ --target trajectory \
            --num_frames 4 --traj_frames 20 --attn_type multi_head \
            --sampling 5 --stop_criteria distance --confidence 250 --map {maps[episodes[i]]} --num_preds 3 --spawn {episodes[i]} {'--command' if command else ''} --sub_command")
